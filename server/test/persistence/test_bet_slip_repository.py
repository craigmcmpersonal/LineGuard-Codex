...
class TestBetSlipRepository(unittest.IsolatedAsyncioTestCase):
    async def test_bet_slip_repository_creates_bet_slip_record_async(self):
        name: str = compose_unique_identifier()
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: Repository = Repository(connection)
            book: Book = await repository.book.create(name)
            ...
            try:
                ...
                bet_slip: BetSlip = await repository.bet_slip.create(
                    user_key,
                    book.key,
                    0,
                    0,
    ...
    async def test_book_repository_finds_bet_slip_record_by_book_identifier_async(self):
        name: str = compose_unique_identifier()
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: Repository = Repository(connection)
            ...
            try:
                ...
                self.assertIsNotNone(bet_slip.key)
                try:
                    retrieved: list[BetSlip] = await repository.bet_slip.find_by_book_identifier(
                        user_key,
                        book.key,
                        external_identifier)
    ...
    async def test_book_repository_finds_bet_slip_record_by_status_async(self):
        name: str = compose_unique_identifier()
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: Repository = Repository(connection)
            ...
            try:
                ...
                self.assertIsNotNone(bet_slip.key)
                try:
                    retrieved: list[BetSlip] = await repository.bet_slip.find_by_status(user_key, bet_slip.status)
                    self.assertIsNotNone(retrieved)
                    if (count := len(retrieved)) != 1:
                        self.fail(count)
                    else:
    ...
    async def test_bet_slip_repository_gets_bet_slip_record_async(self):
        name: str = compose_unique_identifier()
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: Repository = Repository(connection)
            ...
            try:
                ...
                self.assertIsNotNone(bet_slip.key)
                try:
                    retrieved: BetSlip = await repository.bet_slip.try_get(bet_slip.key)
                    self.assertIsNotNone(retrieved)
    ...
    async def test_bet_slip_repository_updates_bet_slip_record_async(self):
        name: str = compose_unique_identifier()
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: Repository = Repository(connection)
            ...
            try:
                ...
                try:
                    ...
                    retrieved: BetSlip = await repository.bet_slip.update(bet_slip)
                    self.assertIsNotNone(retrieved)
