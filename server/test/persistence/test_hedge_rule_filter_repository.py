...
from develop.persistance.model import HedgeRuleFilter
...
class TestHedgeRuleFilterRepository(BaseRepositoryTest):
    async def test_hedge_rule_filter_repository_creates_hedge_rule_filter_record_async(self):
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            hedge_rule_filter: HedgeRuleFilter = await repository.hedge_rule_filter.create(
                                hedge_rule.key,
                                sport_key=sport.key,
                            ...
                            try:
                                ...
                                await repository.hedge_rule_filter.try_delete(hedge_rule_filter)
                        finally:
    ...
    async def test_hedge_rule_filter_repository_finds_hedge_rule_filter_record_async(self):
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: Repository = Repository(connection)
            user_hedge_profile: UserHedgeProfile = await self._create_user_hedge_profile(repository)
            try:
                ...
                try:
                    ...
                    self.assertIsNotNone(hedge_rule_filter.key)
                    try:
                        retrieved: list[HedgeRuleFilter] = await repository.hedge_rule_filter.find(
                            hedge_rule.key
                        )
    ...
    async def test_hedge_rule_filter_repository_gets_hedge_rule_filter_record_async(self):
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: Repository = Repository(connection)
            user_hedge_profile: UserHedgeProfile = await self._create_user_hedge_profile(repository)
            try:
                ...
                try:
                    ...
                    self.assertIsNotNone(hedge_rule_filter.key)
                    try:
                        retrieved: HedgeRuleFilter = await repository.hedge_rule_filter.try_get(
    ...
    async def test_hedge_rule_filter_repository_updates_hedge_rule_filter_record_async(self):
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: Repository = Repository(connection)
            user_hedge_profile: UserHedgeProfile = await self._create_user_hedge_profile(repository)
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        updated: HedgeRuleFilter = await repository.hedge_rule_filter.update(
                            hedge_rule_filter
                        )
