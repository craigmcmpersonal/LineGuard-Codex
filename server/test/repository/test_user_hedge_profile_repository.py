import datetime
import unittest

from uuid import UUID, uuid4

from asyncpg.pool import Pool

from develop.model.user_hedge_profile import UserHedgeProfile
from develop.repository.user_hedge_profile_repository import UserHedgeProfileRepository
from develop.repository.postgresql_pool_factory import create_pool_async
from develop.utility import compose_unique_identifier


class TestUserHedgeProfileRepository(unittest.IsolatedAsyncioTestCase):
    async def test_user_hedge_profile_repository_creates_user_hedge_profile_record_async(self):
        user_key: UUID = uuid4()
        content: str = compose_unique_identifier()
        version: int = 2
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: UserHedgeProfileRepository = UserHedgeProfileRepository(connection)
            user_hedge_profile: UserHedgeProfile = await repository.create(user_key, content, version=version)
            self.assertIsNotNone(user_hedge_profile)
            self.assertIsNotNone(user_hedge_profile.key)
            try:
                self.assertEqual(user_key, user_hedge_profile.user_key)
                self.assertEqual(content, user_hedge_profile.content)
                self.assertEqual(version, user_hedge_profile.version)
                initial_user_hedge_profile: str = user_hedge_profile.model_dump_json()
                print(initial_user_hedge_profile)
            finally:
                await repository.try_delete(user_hedge_profile)

    async def test_user_hedge_profile_repository_finds_user_hedge_profile_record_async(self):
        user_key: UUID = uuid4()
        content: str = compose_unique_identifier()
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: UserHedgeProfileRepository = UserHedgeProfileRepository(connection)
            user_hedge_profile: UserHedgeProfile = await repository.create(user_key, content)
            try:
                retrieved_user_hedge_profile: UserHedgeProfile = await repository.try_find(user_hedge_profile.user_key)
                self.assertEqual(user_key, retrieved_user_hedge_profile.user_key)
                self.assertEqual(content, retrieved_user_hedge_profile.content)
                self.assertEqual(1, retrieved_user_hedge_profile.version)
            finally:
                await repository.try_delete(user_hedge_profile)

    async def test_user_hedge_profile_repository_gets_user_hedge_profile_record_async(self):
        user_key: UUID = uuid4()
        content: str = compose_unique_identifier()
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: UserHedgeProfileRepository = UserHedgeProfileRepository(connection)
            user_hedge_profile: UserHedgeProfile = await repository.create(user_key, content)
            try:
                retrieved_user_hedge_profile: UserHedgeProfile = await repository.try_get(user_hedge_profile.key)
                self.assertEqual(user_key, retrieved_user_hedge_profile.user_key)
                self.assertEqual(content, retrieved_user_hedge_profile.content)
                self.assertEqual(1, retrieved_user_hedge_profile.version)
            finally:
                await repository.try_delete(user_hedge_profile)

    async def test_user_hedge_profile_repository_updates_user_hedge_profile_record_async(self):
        user_key: UUID = uuid4()
        content: str = compose_unique_identifier()
        pool: Pool = await create_pool_async()
        async with pool.acquire() as connection:
            repository: UserHedgeProfileRepository = UserHedgeProfileRepository(connection)
            user_hedge_profile: UserHedgeProfile = await repository.create(user_key, content)
            try:
                self.assertIsNone(user_hedge_profile.valid_to)
                user_hedge_profile.valid_to = datetime.datetime.now(datetime.timezone.utc)
                revised_user_hedge_profile: UserHedgeProfile = await repository.update(user_hedge_profile)
                self.assertEqual(user_key, revised_user_hedge_profile.user_key)
                self.assertEqual(content, revised_user_hedge_profile.content)
                self.assertEqual(1, revised_user_hedge_profile.version)
                self.assertEqual(user_hedge_profile.valid_to, revised_user_hedge_profile.valid_to)
                retrieved_user_hedge_profile: UserHedgeProfile = await repository.try_get(user_hedge_profile.key)
                self.assertEqual(user_key, retrieved_user_hedge_profile.user_key)
                self.assertEqual(content, retrieved_user_hedge_profile.content)
                self.assertEqual(1, retrieved_user_hedge_profile.version)
                self.assertEqual(user_hedge_profile.valid_to, retrieved_user_hedge_profile.valid_to)
            finally:
                await repository.try_delete(user_hedge_profile)


if __name__ == '__main__':
    unittest.main()
